...
class UserAgentPoolTest {
    ...
    void defaultAgentMatchesConfig() {
    ...
    void returnsConstantItself() {
        assertSame(UserAgentPool.at(0), UserAgentPool.at(0),
                "同一索引应返回同一个字符串常量引用");
    }

    @Test
    @DisplayName("EDGE_WINDOWS 就是默认面孔，且与改造前的 UA 逐字一致")
    void defaultFaceIsEdgeAndUnchanged() {
        assertEquals(BilibiliEndpoint.userAgent, UserAgentPool.EDGE_WINDOWS,
                "改动不能悄悄换掉默认面孔的版本号 —— 换了就是行为变更");
        assertSame(UserAgentPool.EDGE_WINDOWS, UserAgentPool.at(0));
    }

    @Test
    @DisplayName("edgeWindows 按版本号拼 UA；版本为空时回落默认面孔")
    void buildsEdgeUserAgent() {
        String ua = UserAgentPool.edgeWindows("140.0.0.0");
        assertTrue(ua.startsWith("Mozilla/5.0 (Windows NT 10.0; Win64; x64)"), ua);
        assertTrue(ua.contains("Edg/140.0.0.0"), "Edge 标识要跟上版本：" + ua);
        assertTrue(ua.contains("Chrome/140.0.0.0"), "真实 Edge 的 Chrome 版本与 Edg 一致：" + ua);
        assertFalse(ua.contains("%s"), "模板占位符必须被替换干净：" + ua);

        // 传空不能拼出畸形 UA，要老老实实回落
        assertEquals(UserAgentPool.EDGE_WINDOWS, UserAgentPool.edgeWindows(null));
        assertEquals(UserAgentPool.EDGE_WINDOWS, UserAgentPool.edgeWindows("   "));
        assertEquals(UserAgentPool.EDGE_WINDOWS, UserAgentPool.edgeWindows(""));
    }
}